...
def flag_format(flag):
    try:
        ...
        if isinstance(flag, str):
            return getattr(fn, flag)
        elif isinstance(flag, list):
            try:
                # For component versions
                if isinstance(flag, list):
                    return fn.component_assigner(flag[0])
            except Exception:
                # For call to worship
                return (getattr(fn, flag[0]), flag[1])
    except Exception:
        return "Not found"
...
def slide_writer(flag, prs):
    ...
    flag_val = flag_format(fn.slide_dict[flag])
    ...
    elif fn.slide_dict[flag] == "goodbye":
        slide = slide_maker(7, prs)

    elif flag in fn.song_list and flag_val != "Not found":
    ...
    elif "prayer" in fn.slide_dict[flag][0]:
        title = fn.slide_dict[flag][1]
        body = ""
        slide = slide_maker(4, prs)
        SlideFill(slide).fill_reading(title, body)
